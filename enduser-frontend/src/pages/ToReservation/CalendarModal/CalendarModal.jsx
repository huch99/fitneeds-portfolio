...
const CalendarModal = ({ sportId, isOpen, onClose, strtDt, endDt, onSelectDate, selectedDate, onProceedToPayment }) => {
    ...
    const isDateActive = (calendarDate) => {
        const dateString = calendarDate.toISOString().split('T')[0]; // "YYYY-MM-DD" 형식
    ...
    const tileDisabled = ({ date: calendarDate, view }) => {
        if (view === 'month') {
            ...
            // 1. 이미 지난 과거 날짜는 비활성화
            if (calendarDate.setHours(0,0,0,0) < today.getTime()) {
                return true; 
            }

            // 2. minDate/maxDate 범위 밖의 날짜는 비활성화
            ...
            // 3. 백엔드에서 가져온 scheduledDates에 없는 날짜는 비활성화
            if (!isDateActive(calendarDate)) {
                return true;
            }
        }
