...
const passApi = {
    ...
    updatePass: (id, data) => {
        console.log("📤 updatePass API 호출 - id:", id, "data:", data);
        console.log("📤 요청 payload:", JSON.stringify(data));
        return api.put(`/user-pass/${id}`, data)
            .catch(err => {
                console.error("❌ updatePass 실패 - status:", err.response?.status);
    ...
    updateStatus: (id, status) => {
        return api.patch(`/user-pass/${id}/status`, {}, { params: { status } });
    },
